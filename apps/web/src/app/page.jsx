...
export default function Page() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const apiUrl = process.env.REACT_APP_API_URL || process.env.VITE_API_URL || 'http://192.168.1.78:3001';
      ...
      // Store both in localStorage AND set the global token
      if (data.sessionToken) {
        localStorage.setItem("admin_token", data.sessionToken);
        setAdminSessionToken(data.sessionToken);
      }
      localStorage.setItem("admin", JSON.stringify(data.admin || { email: normalizedEmail }));
  ...
  const handleSetup = async (e) => {
    ...
    try {
      const apiUrl = process.env.REACT_APP_API_URL || process.env.VITE_API_URL || 'http://192.168.1.78:3001';
      const response = await fetch(`${apiUrl}/api/admin/setup`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ email: normalizedEmail, password }),
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8">
        ...
        <form onSubmit={showSetup ? handleSetup : handleLogin} className="space-y-6">
          <div>
          ...
          >
            {loading ? "Working…" : showSetup ? "Create Owner Account" : "Sign In"}
          </button>

          <div className="flex items-center justify-between text-sm">
            <a className="text-gray-600 hover:text-gray-900" href="/admin/forgot-password">
              Forgot password?
            </a>
            <button
              type="button"
          ...
          {showSetup ? (
            <div className="mt-2 rounded-lg border border-gray-200 bg-gray-50 p-4 text-xs text-gray-600">
              <div className="font-semibold text-gray-800">One-time owner setup</div>
              <div className="mt-1">This only works if there are no admins yet, and only for <strong>ethanserbantes@gmail.com</strong>.</div>
            </div>
          ) : null}
        </form>
      </div>
